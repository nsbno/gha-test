...
async function getEnvironments(): Promise<string[] | undefined> {
...
async function checkTerraformChanges(
  environments: string[]
): Promise<string[]> {
  ...
  for (const env of environments) {
    ...
    try {
      ...
      const awsAccountId =
        process.env.AWS_ACCOUNT_ID || core.getInput('aws-account-id')
      const deploymentRole =
        process.env.AWS_DEPLOYMENT_ROLE_NAME || core.getInput('deployment-role')
      ...
      core.warning(
        `Failed to check for changes in ${env}: ${error instanceof Error ? error.message : error}`
      )
    }
